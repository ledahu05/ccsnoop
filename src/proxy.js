...
export function createProxyServer(options = {}) {
  ...
  const server = http.createServer((creq, cres) => {
    ...
    creq.on('end', () => {
      ...
      // zstd`, and br/zstd have no reliable magic bytes, so `decodeResponseBlob`
      // (report) could not inflate a captured body served in them — usage would
      // read null and waste would flip to a false `cold` (issue #45). Forcing gzip
      // leaves upstream only gzip or plain, the two cases it already handles.
